...
f_wt = 'outputs/checkm2_ecoli_k12_simulation/protein_files/mutated_000000.fna.faa'
f_100 = 'outputs/checkm2_ecoli_k12_simulation/protein_files/mutated_000100.fna.faa'
f_1k = 'outputs/checkm2_ecoli_k12_simulation/protein_files/mutated_001000.fna.faa'
...
data = pl.DataFrame(data, schema={'KO': pl.String, 'length_wt': pl.Int32, 'length_100': pl.Int32, 'length_1k': pl.Int32})
diff = data.filter(pl.col('length_wt') != pl.col('length_1k')).sort('KO')

fig, ax = plt.subplots()
ax.clear()
sns.boxplot(data=diff.melt(id_vars=['KO']), x='variable', y='value', ax=ax, width=.2, boxprops=dict(alpha=.4, facecolor='w'),)
...
fig.savefig(f'plots/check2_{tag}_lengths_by_ko.png')
